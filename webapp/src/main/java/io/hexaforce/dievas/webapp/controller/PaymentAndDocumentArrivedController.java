...
@RestController
@RequestMapping("/api")
public class PaymentAndDocumentArrivedController {
	@Autowired
	private PaymentAndDocumentArrivedService paymentAndDocumentArrived;

	@ResponseBody
	@GetMapping(value = "/payment_and_document_arrived_/")
	public ResponseEntity<String> serviceName() {
		return new ResponseEntity<String>(paymentAndDocumentArrived.serviceName(), HttpStatus.OK);
	}
}
